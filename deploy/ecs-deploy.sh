...
task_def=$(printf "$task_template" $ECS_TASK_DEFINITION_NAME $AWS_ACCOUNT_ID $ECS_REGION $ECR_NAME $VERSION $TASK_CPU $TASK_MEMORY $TASK_MEMORY_RESERVATION $PORT $PORT $ENVIRONMENT $LOG_GROUP $ECS_REGION $LOG_PREFIX $ECR_NAME $COMMAND)
